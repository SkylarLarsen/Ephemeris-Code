...
y_err = []
for i in old_err:
    y_err.append(m.sqrt(i) * 0.0024)
#print(y_err)                                                       #convert error from pixels^2 to arcseconds

average_y = sum(y)/len(y)                                           #average of y data
stdev_y = stat.stdev(y)                                             #stdev of y data
#print("Average:",average_y)
#print("Standard Deviation:",stdev_y)
plt.scatter(t,y)
plt.errorbar(t,y,yerr=y_err, linestyle="None")                      #plot raw data w/ error bars (note: current error bars are too small to be seen)
...
### ECM Fitting the Data ##################################################################################################################################
refCoor = 57368.84735444445       #Given Dec information
mean = 0.0366658
slope = 0.0000184514
period = 365.256
aSin1 = 0.00596366
...
#print(ECM_output)
#plt.scatter(t, ECM_output)               #fit raw data into ECM function, get ECM outputs
...
#print(data)
#plt.scatter(t,data)                        #subtract ECM outputs from raw data
...
print('amplitude:', peak_amp)       #highest peak of LS fit
print(low_amp)                      #lowest peak of LS fit

for offset in [-1, 0, 1]:
    ax.errorbar(phase + offset, data, y_err, fmt='.',
               color='gray', ecolor='lightgray', capsize=0);      #plot points
ax.plot(phase_model, mag_model, '-k', lw=2)                       #plot model
ax.set(xlim=(0, 1),
       xlabel='phase',
       ylabel='difference')                                       #axis labels
